...
void init(data_t x[Nsta], data_t fx[Nsta], data_t hx[Mobs], data_t F[Nsta][Nsta],
			data_t H[Mobs][Nsta], data_t P[Nsta][Nsta], data_t Q[Nsta][Nsta],
				data_t R[Mobs][Mobs], port_t params[(((2*Nsta)+2+Mobs)*Nsta)+Mobs+2])
{

	#pragma HLS INLINE off

	data_t local_mem[(((2*Nsta)+2+Mobs)*Nsta)+Mobs+2];
	data_t qval, rval;
	static const int ptotal = (((2*Nsta)+2+Mobs)*Nsta)+Mobs+2;
...
// top function
void gps_ekf(port_t *xin, port_t params[(((2*Nsta)+2+Mobs)*Nsta)+Mobs+2], port_t *output,
				port_t pout[Nsta*Nsta], int datalen)
{
    ...
    for (int i=0; i<datalen; i++) {  //datalen
        ...
        for (int j=0; j<num_inputs; j++) {
			#pragma HLS PIPELINE
			local_mem[j].V = xin[i*num_inputs + j].range(bit_width-1, 0);
		}
		...
		for (int j=0; j<Nsats; j++)	{
            for (int k=0; k<Nxyz; k++) {
				#pragma HLS PIPELINE
				SV_Pos[j][k] = local_mem[j*Nxyz + k];
            }
        }
        for (int j=0; j<Nsats; j++) {
			#pragma HLS PIPELINE
            SV_Rho[j] = local_mem[j + num_inputs - Nsats];
        }
